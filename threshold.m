...
fileFolder=fullfile('D:/doc/4/graduation/data/0001/DICOMIMG');
dicomlist = dir(fullfile(fileFolder));

for cnt = 89 : 89    
    I{cnt} = dicomread(fullfile(fileFolder,dicomlist(cnt).name));
    info=dicominfo(fullfile(fileFolder,dicomlist(cnt).name)); % image info
    Y=imadjust(I{cnt});
    %figure, imshow(Y,[])
    %title('Imadjust')
